					...
					while (server_i.hasNext()) {
						...
						try {
							...
							if (!flag_v) {
								...
								if(client == null) {
									client = new RpcClient(server, serverPort, 3000, token, key.getPublicKey());
...
//								}
	 ...
	 * @param msg 放在不同task上的msg对象一定不能相同，需要克隆出新对象。
	 * @throws
	 */
	public void request(RPCMessage msg) {
//		System.out.println("start request MessageId=" + msg.getMessageId());
		//循环给全部在线任务提交消息
		
		taskList.forEach((k ,v) -> {
			if(v != null && v.status) {
//				System.out.println("添加一个消息: id=" + id);
				msg.setTarget(k);
...
//				System.out.println("丢弃消息");
	...
	public class Task{
		...
		public Task(String serverHost, int serverPort, Cipher cipher, String token, Key key, String connectionId, int workerNum) {
			...
			try {
				this.rpcPool = new RpcClientPool(serverHost , serverPort, workerNum * 2, token, key.getPublicKey());
			...
			request = new Request(this, serverHost, serverPort, cipher, token, key, connectionId, 3, serverPublicKey);
			resphone = new Resphone(this, serverHost, connectionId, key, cipher, 3, serverPublicKey);
		}
